...
module.exports.clearCache = (prefix = '', key = '', cb = () => {}) => {
    if (prefix && key) return cache.del(key, prefix).then(() => cb(null)).catch(err => cb(err));
    if (prefix) return cache.delByPrefix(prefix).then(() => cb(null)).catch(err => cb(err));

    return cache.clear().then(() => cb(null)).catch(err => cb(err));
};
